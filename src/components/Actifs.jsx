...
const CAT_COLORS = { immobilier: '#1A201C', financier: '#1F6B4A', professionnel: '#4A544D', exotique: '#9333EA' };
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '36px 24px 100px' }} className="fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
        <div>
          <p style={{ color: '#1F6B4A', fontSize: 12, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 6 }}>Mes actifs</p>
          <h1 className="font-serif" style={{ color: '#1A201C', fontSize: 34, margin: 0 }}>{povTitle(pov, userProfile)}</h1>
          <p style={{ color: '#5C635E', marginTop: 6, fontSize: 14 }}>Total estimé : <strong style={{ color: '#1A201C' }}>{euro(totals.total)}</strong></p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 16, marginBottom: 28 }}>
        {['immobilier', 'financier', 'professionnel', 'exotique'].map(cat => (
          <div key={cat} className="card" onClick={() => setFilter(filter === cat ? 'tous' : cat)}
            ...
            <p style={{ color: '#5C635E', fontSize: 12, marginTop: 8, textTransform: 'capitalize' }}>{cat}</p>
            <p style={{ fontWeight: 700, color: '#1A201C', fontSize: 21, margin: '2px 0' }}>{euro(totals[cat])}</p>
            <p style={{ color: '#5C635E', fontSize: 11, margin: 0 }}>{povActifs.filter(a => a.categorie === cat).length} actif(s)</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
        {[['tous', 'Tous'], ['immobilier', '🏠 Immobilier'], ['financier', '💰 Financier'], ['professionnel', '🏢 Professionnel'], ['exotique', '💎 Exotique']].map(([id, label]) => (
          <button key={id} onClick={() => setFilter(id)} style={{ padding: '8px 16px', borderRadius: 20, fontSize: 13, cursor: 'pointer', border: 'none', fontFamily: 'DM Sans, sans-serif', transition: 'all 0.2s', background: filter === id ? '#1A201C' : 'white', color: filter === id ? 'white' : '#4A544D', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {Object.entries(byCategorie).map(([cat, list]) => (
        <div key={cat} style={{ marginBottom: 32 }}>
          <h2 className="font-serif" style={{ color: '#1A201C', fontSize: 22, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {list.map(actif => (
              <div key={actif.id} className="card" style={{ padding: 24, borderLeft: `4px solid ${CAT_COLORS[actif.categorie]}` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                  <h3 style={{ fontWeight: 600, color: '#1A201C', fontSize: 15, margin: 0, lineHeight: 1.3 }}>{actif.nom}</h3>
                  <span style={{ background: '#F6F4ED', color: '#4A544D', fontSize: 11, padding: '3px 9px', borderRadius: 6, whiteSpace: 'nowrap', marginLeft: 8 }}>{actif.type}</span>
                </div>
                <p className="font-serif" style={{ color: '#1F6B4A', fontSize: 28, fontWeight: 700, margin: '0 0 14px' }}>{euro(actif.valeur)}</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                  {[
                    ...
                    actif.pays ? ['Pays', actif.pays, actif.pays !== 'France' ? '#D99A2B' : null] : null,
                    actif.beneficiaire ? ['Bénéficiaire', actif.beneficiaire, null] : null,
                  ].filter(Boolean).map(([label, val, color]) => (
                    <div key={label} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                      <span style={{ color: '#5C635E' }}>{label}</span>
                      <span style={{ color: color || '#1A201C', fontWeight: 500 }}>{color ? `⚠ ${val}` : val}</span>
                    </div>
                  ))}
                  {actif.credit && (
                    <div style={{ marginTop: 6, background: '#FAEDE7', borderRadius: 6, padding: '6px 10px' }}>
                      <span style={{ color: '#C2502F', fontSize: 12 }}>⚠ Crédit en cours</span>
                    </div>
                  )}
                  {actif.note && (
                    <div style={{ marginTop: 6, background: '#FBF5E3', borderRadius: 6, padding: '6px 10px' }}>
                      <span style={{ color: '#D99A2B', fontSize: 12 }}>ℹ {actif.note}</span>
                    </div>
                  )}
                </div>
              </div>
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Ajouter un actif">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {[
            ...
            <div key={f.key}>
              <label style={{ color: '#4A544D', fontSize: 13, display: 'block', marginBottom: 6 }}>{f.label}</label>
              <input type={f.type} value={form[f.key]} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))} placeholder={f.placeholder}
                style={{ width: '100%', border: '1px solid #DDD8C9', borderRadius: 8, padding: '10px 14px', fontSize: 14, fontFamily: 'DM Sans, sans-serif' }} />
            </div>
          ))}
          {[
            ...
            <div key={key}>
              <label style={{ color: '#4A544D', fontSize: 13, display: 'block', marginBottom: 6 }}>{label}</label>
          ...
          <p style={{ color: '#5C635E', fontSize: 12, margin: 0 }}>Attribué à {resolvePovLabel(pov, userProfile)} (point de vue actuel)</p>
